...
OPTIMIZATION_LEVEL_ORDER = ["O0", "IC O0", "DII O0 combined", "IC + DII O0 combined", "O3", "IC O3", "DII O3 combined", "IC + DII O3 combined"]
...
def plot_metric_on_ax(ax, data, benchmarks, metric_col, y_label):
    optimization_levels = sorted(
        data.index.get_level_values("optimization_level").unique(),
        key=OPTIMIZATION_LEVEL_ORDER.index
    )
...
def create_single_metric_plot(data, compiler, metric_col, y_label, filename_suffix, benchmarks_list):
    ...
    mid_point = math.ceil(len(available_benchmarks) / 2)
...
def main():
    ...
    for compiler in df["compiler"].unique():
        ...
        processed = process_benchmark_set(compiler_data, compiler, EXECUTION_TIME_BENCHMARKS + THROUGHPUT_BENCHMARKS, "All")

        combined_aggregated = aggregate_data(processed)
        
        avg_speedup_o3 = combined_aggregated.groupby(level='optimization_level')['speedup_o3'].agg(['mean', 'std']).round(4)
        avg_norm_binary_size_o3 = combined_aggregated.groupby(level='optimization_level')['norm_binary_size_o3'].agg(['mean', 'std']).round(4)
